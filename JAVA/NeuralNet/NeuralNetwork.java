...
public class NeuralNetwork {
  ...
  /*
   * all the weight of the neural network
   * 
   * don't change the size of the matrix or the neural network will break
   */
  public Matrix[] weights;
  /*
   * all the values of the neural network
   * 
   * don't change the size of the matrix or the neural network will break
   */
  public Matrix[] values;
  /*
   * all the biais of the neural network
   * 
   * don't change the size of the matrix or the neural network will break
   */
  public Matrix[] biais;
  ...
  public int MinOutputIndex() {
    return(values[layer-1].min()[1]);
  }
  ...
  public Function<Float,Float> getActivation(int i){
    return(activation[i]);
  }
  private Function<Float, Float>[] derivative;
  public Function<Float,Float> getDerivative(int i){
    return(derivative[i]);
  }
  ...
  public void crossOver(NeuralNetwork parent1,NeuralNetwork parent2,float swapProb) {
    apply(parent1);
    for(int i=0;i<layer-1;i++) {
      weights[i].crossover(parent2.weights[i],swapProb);
      biais[i].crossover(parent2.biais[i],swapProb);
    }
  }
  ...
  public void mutate(float radiation,float start,float end) {
    for(int i=0;i<layer-1;i++) {
      weights[i].mutate(radiation, start, end);
      biais[i].mutate(radiation, start, end);
    }
  }
  
  public void mutate(float radiation) {
    mutate(radiation,-1,1);
  }
  ...
  public void compute(){
    for(int i=0;i<weights.length;i++){
      ...
      values[i+1].add(biais[i]);
      values[i+1].map(activation[i]);
      if(activation[i]==functions.softMax.Function) {
        values[i+1].normalize();
      }
    }
  }
  ...
  public void backPropagation(Object target) throws IllegalArgumentException,IndexOutOfBoundsException{
    ...
    if(target.getClass() != Matrix.class && target.getClass() != float[].class) {
      throw new IllegalArgumentException("input must be float[] or matrix");
    }
    ...
    for(int i=0;i<weights.length;i++) {
      ...
      Nsum.fusion(Err);
      Nsum.clamp(100000);// to avoid Nan issue
      
      Nsum.factor(learningRate);
      
      Matrix In = values[values.length-2-i].clone();
      In = In.transpose();
      
      Matrix Delta = Nsum.product(In);
      
      weights[weights.length-1-i].add(Delta);//5
      biais[biais.length-1-i].add(Nsum);
      
      Err = oldW.product(Err);
    }
    experience++;
  }
  /**
  ...
  //use ArrayList as data base
  public void backPropagation(ArrayList<float[][]> dataBase, int train_number) {
    for(int i=0;i<train_number;i++) {
      int index=rand.nextInt(dataBase.size());
      backPropagation(dataBase.get(index)[0],dataBase.get(index)[1]);
    }
  }
  
  public void backPropagation(ArrayList<float[][]> dataBase, float ratio){
    
    for(float[][] data:dataBase) {
      backPropagation(data[0],data[1]);
    }
  }
  
  /**
   * 
   * @return the neural Network converted in NeuronNetwork
   */
  public NeuronNetwork toNeuronNetwork() {
	  return null;
  }
  ...
  public void apply(NeuralNetwork nn) throws IndexOutOfBoundsException{
    if(layer!=nn.getLength()){
      throw new IndexOutOfBoundsException("the neural network doesn't have the same amount of layer");
    }
    for(int i=0;i<layer;i++){
      if(nn.getSructure(i)!=structure[i]) {
        throw new IndexOutOfBoundsException("different size !! layer : "+i);
      }
    }
    for(int i=0;i<layer-1;i++) {
      weights[i]=nn.weights[i].clone();
      biais[i]=nn.biais[i].clone();
      activation[i]=nn.getActivation(i);
      derivative[i]=nn.getDerivative(i);
    }
  }
}